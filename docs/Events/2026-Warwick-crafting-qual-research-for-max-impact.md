---
layout: default
title: Workshop on Crafting Qualitative Research for Maximum Impact [Warwick Business School, 21/04/2026] 
nav_order: 2026c
parent: Upcoming Events
grand_parent: Events
has_children: false
has_toc: true
---

# Workshop on Crafting Qualitative Research for Maximum Impact [Warwick Business School, 21/04/2026]
Presented by Kevin Corley, Charlotte Cloutier, and Ann Langley 

Are you a qualitative scholar aiming to craft well-designed and rigorously executed projects? Kevin, Charlotte, and Ann have teamed up to write a book detailing their guidance on how to craft qualitative research for maximum impact.

In working with and talking to other qualitative researchers, some of the most challenging issues and questions that arise revolve around how to develop meaningful theoretical insights from the empirical analysis of qualitative data. Part of the reason why these issues and questions are so challenging is because crafting meaningful theoretical insights involves intuition and humble confidence, two skills that can really only be built through experience. But it helps tremendously if that experience is born out of well-designed and rigorously executed qualitative research projects. 

In this session, they will:

✅ Share their orienting framework on crafting qualitative research for maximum impact

✅ Walk through how their framework can be used to explore some of the fundamental elements of developing rigorous yet relevant contributions in qualitative research.

✅ Encourage discussion and learning from others in our community


📅 Date & Time: Tuesday, April 21, 2026 | Time TBD (BST)

📍 Location: University of Warwick - Warwick Business School, Coventry, UK

🔗 Registration: TBD


Questions? Reach out to the organizing committee through the L+QC committee at londonqualitativecommunity@gmail.com, we’re happy to help! 
